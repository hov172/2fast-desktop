# 2fast 1.5.2 for macOS

The app is named **2fast**, version **1.5.2**, build **152**. Download
`2fast-macos-universal.dmg` from the private repository's
[release page](https://github.com/hov172/2fast-desktop/releases/tag/v1.5.2).
...
export APPLE_DISTRIBUTION_PROFILE="$HOME/Library/Developer/Xcode/UserData/Provisioning Profiles/<direct-profile>.provisionprofile"
...
[release notes](docs/RELEASE-v1.5.2.md).
...
ID certificate SHA-1 and `APPLE_DISTRIBUTION_PROFILE` with the path to the
"Mac Team Direct Provisioning Profile" for `jpweber.it.Project2FA.Uno`. The
keychain entitlements are restricted, so without that embedded profile macOS
refuses to launch the signed app (`RBSRequestErrorDomain Code=5`). The

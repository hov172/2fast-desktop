    ...
    public partial class EditAccountViewModelBase : ObservableObject
    {
        ...
        public async Task<bool> SearchAccountFonts(string senderText)
        {
            if (string.IsNullOrEmpty(senderText) == false && senderText.Length >= 2 && senderText != Strings.Resources.AccountCodePageSearchNotFound)
            {
                ...
                var tempList = DataService.Instance.FontIconCollection.Where(x => Project2FA.Converters.IconNameLookup.Normalize(x.Name).Contains(iconQuery, System.StringComparison.Ordinal)).ToList();
#else

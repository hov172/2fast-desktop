...
namespace Project2FA.ViewModels
{
    ...
    public partial class EditAccountContentDialogViewModel : EditAccountViewModelBase, IDialogInitialize
    {
        ...
        {
            var original = (TwoFACodeModel)Model.Clone();
            try
            {
                Model.Issuer = Issuer; Model.Label = Label;
                Model.AccountIconName = AccountIconName; Model.Notes = Notes;
                if (!await DataService.Instance.WriteLocalDatafile())
                    throw new IOException("The edits could not be saved. Your previous account details were restored.");
            }
            catch
            {
                Model.Issuer = original.Issuer; Model.Label = original.Label;
                Model.AccountIconName = original.AccountIconName; Model.Notes = original.Notes;
                throw;
            }
        }

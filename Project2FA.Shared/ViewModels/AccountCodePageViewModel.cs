...
namespace Project2FA.ViewModels
{
...
#endif
    {
...
#if __ANDROID__ || __IOS__ || TWOFAST_DESKTOP
        ...
        private bool timerTickRunning;
        public void StopAccountTimers()
        {
            timerGeneration++;
        ...
        private async void TOTPTimer(object sender, object e)
        {
            ...
            try { await TOTPTimerTask(); }
            finally { timerTickRunning = false; }
        }

        private async Task TOTPTimerTask()
        {
#if TWOFAST_DESKTOP
            // Recompute from the clock on every tick. A delayed UI tick can skip
            // the old narrow rollover window and otherwise leave an expired OTP.
            for (int i = 0; i < TwoFADataService.Collection.Count; i++)
                await TwoFADataService.GenerateTOTP(i);
        ...
        public async Task SetFavouriteCommandTask(object parameter)
        {
            ...
            {
                bool previous = model.IsFavourite;
                model.IsFavourite = !previous;
                if (!await TwoFADataService.WriteLocalDatafile()) { model.IsFavourite = previous; return; }
        ...
        public async Task<bool> CopyCodeToClipboardCommandTask(TwoFACodeModel model)
        {
            ...
            int currentIndex = TwoFADataService.Collection.IndexOf(model);
            if (currentIndex < 0) return false;
            await TwoFADataService.GenerateTOTP(currentIndex);
#endif
            try
            {
        ...
        {
            var param = new NavigationParameters();
            param.Add("ManualInput", isManualInput);
            await NavigationService.NavigateAsync(nameof(AddAccountPage), param);
        }
#endif

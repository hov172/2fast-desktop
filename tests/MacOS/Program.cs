...
if (args.Contains("--inspect-private-qr"))
{
    ...
    bool valid = StrictProject2FAParser.TryParse(payload, out var imported, out var error);
...
Check(StrictProject2FAParser.TryParse(prefix + "&algorithm=SHA512&digits=8&period=60", out _), "supported custom TOTP settings");
Check(StrictProject2FAParser.TryParse("otpauth://totp/A%26B:a%2Bb%40example.com?secret=JBSWY3DPEHPK3PXP&issuer=A%26B", out pairs)
...
Check(DesktopMobileId.ComputeOtp(OtpNet.Base32Encoding.ToBytes(mobileValues["secret"]), 60, 6, false, DateTimeOffset.FromUnixTimeSeconds(1200000000)) == "473642", "MobileID fixed-offset OTP fixture");
Check(!StrictProject2FAParser.TryParse(mobile.Replace("&ac=24681357", ""), out _, out var importError) && importError == "Authorization code required", "missing authorization code requests input");
Check(StrictProject2FAParser.TryParse(mobile.Replace("&ac=24681357", ""), out _, out _, "24681357"), "separate authorization code imports");
Check(!StrictProject2FAParser.TryParse(mobile, out _, out _, "incorrect"), "wrong authorization code rejected");
Check(StrictProject2FAParser.TryParse(mobile + "&regurl=https%3A%2F%2Fexample.com", out pairs) && pairs.Any(p => p.Key == "importnotice") && !pairs.Any(p => p.Key == "regurl"), "offline import discards push registration and labels the limitation");
Check(!StrictProject2FAParser.TryParse(mobile + "&sn=999", out _), "duplicate MobileID serial rejected");
Check(!StrictProject2FAParser.TryParse(mobile.Replace("sn=123456789", "sn=123456788"), out _), "serial checksum rejected");
string ocra = "otpauth://ocra/Test?secret=JBSWY3DPEHPK3PXP&suite=OCRA-1:HOTP-SHA1-6:QN08-T1M";
Check(StrictProject2FAParser.TryParse(ocra, out pairs) && pairs.Any(x => x.Key == "ocrasuite"), "OCRA QR profile preserved");
Check(!StrictProject2FAParser.TryParse(ocra + "&period=30", out _), "inconsistent OCRA timing rejected");
Check(StrictProject2FAParser.TryParse("otpauth://totp/Standalone?secret=JBSWY3DPEHPK3PXP", out pairs) && pairs.Single(x => x.Key == "label").Value == "Standalone", "QR without issuer has a usable account label");

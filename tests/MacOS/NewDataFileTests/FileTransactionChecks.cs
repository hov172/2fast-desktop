...
internal static class FileTransactionChecks
{
    internal static async Task Run()
    {
        ...
        try
        {
            ...
            await DesktopFileTransaction.Replace(path, "new ciphertext", () => credential = "new", () => credential = "old");
            if (File.ReadAllText(path) != "new ciphertext" || credential != "new") throw new Exception("Commit failed");
            try { await DesktopFileTransaction.Replace(path, "bad candidate", () => { credential = "bad"; throw new IOException("synthetic settings failure"); }, () => credential = "new"); }
            catch (IOException) { }
            ...
            try
            {
                await DesktopFileTransaction.Replace(path, "candidate", () => credential = "bad", () => credential = "new", async (target, text) =>
                {
            ...
            try { await DesktopFileTransaction.Replace(path, "candidate", () => { }, () => { }, (_, _) => throw new IOException("unavailable disk")); }
            catch (IOException) { }
            ...
            await DesktopFileTransaction.CopyNew(path, backup);
            await File.WriteAllTextAsync(path, "later ciphertext");
            try { await DesktopFileTransaction.CopyNew(path, backup); throw new Exception("Overwrite allowed"); } catch (IOException) { }
            ...
            if (DesktopFileTransaction.FileName(" name ") != "name.2fa") throw new Exception("Filename normalization failed");
            foreach (var invalid in new[] { "../outside", "", "a/b", "a\\b" })
                try { DesktopFileTransaction.FileName(invalid); throw new Exception("Invalid filename accepted"); } catch (ArgumentException) { }

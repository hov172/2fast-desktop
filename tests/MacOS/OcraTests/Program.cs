...
for (int i = 0; i < 10; i++) Check(DesktopOcra.Compute(key20, new string((char)('0' + i), 8), DateTimeOffset.UnixEpoch, "OCRA-1:HOTP-SHA1-6:QN08"), expected1[i]);
var timestamp = DateTimeOffset.FromUnixTimeSeconds(0x132d0b6L * 60);
Check(DesktopOcra.Compute(key20, "12345678", timestamp), "866866"); // Independent Python HMAC fixture for the requested profile.
var expected2 = new[] { "95209754", "55907591", "22048402", "24218844", "36209546" };
for (int i = 0; i < 5; i++) Check(DesktopOcra.Compute(key64, new string((char)('0' + i), 8), timestamp, "OCRA-1:HOTP-SHA512-8:QN08-T1M"), expected2[i]);
foreach (var invalid in new[] { "", "123456789", "abcdefgh", "１２３", "-1", "1 2" })
{
    try { DesktopOcra.Compute(key20, invalid, timestamp); throw new Exception("Invalid challenge accepted"); }
    catch (ArgumentException) { passed++; }
}
Check(DesktopOcra.Compute(key20, "00000001", timestamp), DesktopOcra.Compute(key20, "1", timestamp));
